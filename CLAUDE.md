...
12. **Hiệu năng:** `compression` (gzip/br) bật ở `server.js`; `express.static` có cache
    (CSS/JS 7 ngày, ảnh 30 ngày); ảnh sản phẩm nén w=800/q60 (~2.4MB tổng); ảnh hover card
    dùng `data-src` (JS swap khi hover — mobile không tải); hero `fetchpriority="high"`.
    **Cache-busting:** CSS/JS gắn `?v=<mtime>` (ASSET_VER trong server.js) — BẮT BUỘC vì cache dài hạn.
    **Google Analytics 4** tuỳ chọn qua `GA_MEASUREMENT_ID` trong `.env` (tắt mặc định).
13. **Yêu thích & Đã xem** (client-side, localStorage — không cần đăng nhập): nút `[data-wish]`
    trên card/trang SP, badge header, trang `/yeu-thich`, mục "Sản phẩm đã xem" (`[data-recent-section]`).
    Logic trong `main.js` (keys `dc_wish`, `dc_recent`); render thẻ động qua API
    `GET /api/the-san-pham?ids=` → `partials/card-list.ejs` (render `layout:false`).
14. **Biến thể sản phẩm:** `product.options = [{name, values[]}]` (gán trong `seed.js` cho 6 SP).
    Giỏ hàng đổi shape → `{ [lineKey]: {productId, quantity, variant} }` (lineKey = productId +
    biến thể; helper `lineKey/variantKey/variantLabel` ở util.js). Tương thích ngược giỏ cũ (số).
    Biến thể hiển thị ở giỏ/checkout/đơn/email/admin. Card có options → nút "Chọn tuỳ chọn" (link SP).
15. **Email xác nhận đơn:** `src/mailer.js` (nodemailer). Gọi trong `checkout.js` sau khi tạo đơn
    (fire-and-forget). Bật khi có `SMTP_HOST/USER/PASS` trong `.env`; chưa cấu hình → bỏ qua êm.
16. **Meta Pixel:** `FB_PIXEL_ID` trong `.env` (tắt mặc định). Snippet PageView ở layout;
    AddToCart (main.js, submit form thêm giỏ); Purchase (order-success.ejs). GA4 cũng bắn purchase.

**Dependencies mới:** `compression`, `nodemailer` → deploy cần `npm install` (đã có bước này).
